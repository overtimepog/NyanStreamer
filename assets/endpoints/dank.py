...
class Dank():
    ...
    def generate(self, avatars, text, usernames, kwargs):
        ...
        horn = Image.open(('assets/assets/dank/horn.bmp'))\
            .convert('RGBA')\
            .resize((100, 100))\
            .rotate(315, resample=Image.BICUBIC)
        horn2 = ImageOps.mirror(horn.copy().resize((130, 130)).rotate(350, resample=Image.BICUBIC))
        hit = Image.open(('assets/assets/dank/hit.bmp')).convert('RGBA').resize((40, 40))
        gun = Image.open(('assets/assets/dank/gun.bmp')).convert('RGBA').resize((250, 205))
        faze = Image.open(('assets/assets/dank/faze.bmp')).convert('RGBA').resize((60, 40))
