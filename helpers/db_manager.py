...
async def clean_inventory() -> int:
    ...
    async with aiosqlite.connect("database/database.db") as db:
        ...
        cur = await db.execute("SELECT user_id, item_id AS id_type FROM inventory WHERE item_id NOT IN (SELECT item_id FROM basic_items) UNION SELECT user_id, chest_id AS id_type FROM inventory WHERE chest_id NOT IN (SELECT chest_id FROM chests)")
