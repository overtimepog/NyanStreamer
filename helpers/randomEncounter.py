...
async def random_encounter(ctx: Context):
    ...
    embed = discord.Embed(title=enemy[1], description=enemy[5], color=0x00ff00)
    embed.set_thumbnail(url=emoji_url)
    embed.add_field(name="Health", value=enemy[2], inline=True),
    dungeon = await get_dungeon_channel(ctx)
    await dungeon.send(embed=embed)
    return enemy

#loop it
async def loop_random_encounter(ctx: Context):
